...
- **2026-06-25** — **Login no dashboard** (`/login`, `before_request` exige sessão; `/health` e `/login` públicos; APIs JSON sem sessão → 401). Credenciais via `.env` (`AUTH_EMAIL`/`AUTH_PASSWORD`), comparadas em tempo constante (`hmac.compare_digest`); senha **nunca** versionada. Botão "Sair" no header. *Motivo:* dashboard não tinha auth e o deploy expõe a app.
